...
export default function StockComparison() {
  ...
  const { internal, sale } = useMemo(
    () => ({
      internal: products.filter(
        (product) => !isForSale(product)
      ),
  ...
  if (loading) {
    return (
      <div className="panel state">
        Carregando comparação do estoque...
      </div>
    );
  }
    ...
    <div className="stock-comparison">
      <div className="page-heading">
        <div>
          ...
          <p>
            Compare os materiais de uso interno
            com os materiais destinados à venda.
          </p>
        </div>
      </div>

      {/* USO INTERNO */}
      <section className="panel table-panel comparison-panel">
        <div className="comparison-header">
          ...
          <h2>
            Materiais que não serão vendidos
          </h2>
        </div>

        <div className="comparison-count">
          <strong>
            {internal.length} itens
          </strong>
